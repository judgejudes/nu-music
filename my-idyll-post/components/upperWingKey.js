...
import upperWing from '../data/upperWing';
...
const UpperWingKey = ({ }) => {
    ...
    useEffect(() => {
        ...
        if (upperWing && d3Container.current) {
            const svg = d3.select(d3Container.current);
            const flower = 100
            // console.log(flower)

            var g = svg.selectAll('g')
            .data(upperWing).enter().append('g')
